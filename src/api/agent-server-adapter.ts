...
function getConversationSecurityAnalyzer(conversationSettings: SettingsRecord) {
  switch (conversationSettings.security_analyzer) {
    case "llm":
      // Roy 的治理層(2026-09-01):"llm" 這個既有選項改接 Roy 自己的
      // path/payload 治理分析器,不再用 OpenHands 內建的
      // LLMSecurityAnalyzer——後者靠 LLM 自行判斷風險,前者用確定性規則
      // (workspace 內外/是否為終端機指令)判斷,對應 dsh-authz-roles 的
      // pathWithin matcher。詳見 openhands-sdk-governed 的
      // openhands/sdk/security/roy_governance.py。
      return { kind: "RoyPathPayloadSecurityAnalyzer" };
    case "pattern":
