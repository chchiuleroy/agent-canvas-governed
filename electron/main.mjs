...
// ── Roy 的治理層接線(2026-09-01) ──────────────────────────────────────────
// 讓這個桌面 app 預設吃已經加了 RoyPathPayloadSecurityAnalyzer 的
// software-agent-sdk checkout,而不是官方 PyPI 版本;LLM 則指向本機
// vllm_toolcall_proxy.py(修補 Qwen XML 工具呼叫格式)。
// 只在使用者沒有自己設定同名環境變數時才套用預設值,不覆蓋既有設定。
//
// 打包後(app.isPackaged):走 extraResources 打包進安裝檔的
// <Resources>/openhands-sdk-governed/,跟 injectBundledUv()/injectBundledNode()
// 用同一套 process.resourcesPath 慣例。
// 開發模式(dev,npm run desktop):相對於這支檔案自己的位置往上兩層找
// openhands-sdk-governed/(跟 agent-canvas-source 同層的 sibling 目錄),
// 不寫死任何人的個人帳號路徑——換一台機器只要保持同樣的目錄結構就能跑。
const governedSdkPath = app.isPackaged
...
const ROY_GOVERNANCE_DEFAULTS = {
  ...
  LLM_API_KEY: "noauth",
  // 真實上游網域不寫進原始碼(repo 是 public,寫死等於公開一個無驗證的推論端點)。
  // 開發機上用系統環境變數 VLLM_PROXY_UPSTREAM_URL 設定真實值即可覆蓋這個佔位符。
  VLLM_PROXY_UPSTREAM_URL: "https://your-vllm-server.example.com",
};
for (const [key, value] of Object.entries(ROY_GOVERNANCE_DEFAULTS)) {
  if (!process.env[key]) process.env[key] = value;
}

// 跟著 app 生命週期自動起停,不需要使用者另外手動開一個視窗跑 proxy。
// 必須在 startStack() 之前啟動,agent-server 的 LLM_BASE_URL 一開始就指向它。
const vllmToolcallProxy = startVllmToolcallProxy({
  port: 8899,
  upstreamBaseUrl: process.env.VLLM_PROXY_UPSTREAM_URL,
  log: (msg) => console.log(`[desktop] ${msg}`),
});
// 獨立掛一個 SIGTERM listener 只管關 proxy,跟既有的子行程清理邏輯
